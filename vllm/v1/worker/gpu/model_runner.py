...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def execute_model(
        ...
        if not dummy_run:
            ...
            #
            # Solo se escribe el CONTENIDO del buffer persistente; la vista la
            # calcula el forward sobre el atributo que se ato en la construccion.
            # Eso es lo que mantiene esto vivo cuando el lote lo sirve un grafo
            # CUDA ya capturado — antes se pasaba un tensor NUEVO por paso y en
            # replay no se leia jamas.
            if refusal_projection.is_enabled():
                ...
                # El drafter MTP va al global a proposito (ver fill_draft_neutral):
                # tiene dos layouts de fila y adivinar cual toca seria reintroducir
                # el fallo por el otro lado. Cuesta acceptance, no correccion.
                self.refusal_state.fill_draft_neutral(refusal_projection.get_lambda())
        else:
            # Dummy/profile run: los dos buffers al global. Sin esto se consume el
            # contenido RANCIO del ultimo paso real.
            if refusal_projection.is_enabled():
