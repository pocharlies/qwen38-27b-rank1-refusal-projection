...
def _trim_wrapping_newlines(value: str) -> str:
    ...
    Backport of upstream vLLM PR #48846 (merged 2026-07-17), re-derived against the
    0.25.2 parser: the file was rewritten between 0.23.1 and 0.25.2 (8.9K -> 18.2K,
    it now carries the thinking config), but the bug is untouched -- the two
    `value.strip()` below are still there.

    The unconditional strip destroys meaningful whitespace in tool arguments -- most
    visibly the leading indentation of the first line -- which breaks exact-match edit
    tools in agentic clients and makes them retry the same tool call forever. See also
    upstream issue #48753.
    """
    if value.startswith("\n"):
        value = value[1:]
    if value.endswith("\n"):
